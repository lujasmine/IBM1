...
package ucl.team10.anomaly.libsvm;
import java.io.*;
import java.util.*;
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte underscore=y[i]; y[i]=y[j]; y[j]=underscore;} while(false);
		do {double underscore=G[i]; G[i]=G[j]; G[j]=underscore;} while(false);
		do {byte underscore=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=underscore;} while(false);
		do {double underscore=alpha[i]; alpha[i]=alpha[j]; alpha[j]=underscore;} while(false);
		do {double underscore=p[i]; p[i]=p[j]; p[j]=underscore;} while(false);
		do {int underscore=active_set[i]; active_set[i]=active_set[j]; active_set[j]=underscore;} while(false);
		do {double underscore=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=underscore;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte underscore=y[i]; y[i]=y[j]; y[j]=underscore;} while(false);
		do {double underscore=QD[i]; QD[i]=QD[j]; QD[j]=underscore;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte underscore=sign[i]; sign[i]=sign[j]; sign[j]=underscore;} while(false);
		do {int underscore=index[i]; index[i]=index[j]; index[j]=underscore;} while(false);
		do {double underscore=QD[i]; QD[i]=QD[j]; QD[j]=underscore;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int underscore=perm[i]; perm[i]=perm[j]; perm[j]=underscore;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
	...
	private static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
		...
		{
			do {int underscore=label[0]; label[0]=label[1]; label[1]=underscore;} while(false);
			do {int underscore=count[0]; count[0]=count[1]; count[1]=underscore;} while(false);
			for(i=0;i<l;i++)
			{
				if(data_label[i] == 0)
					data_label[i] = 1;
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int underscore=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=underscore;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
